...
if ($method == 'fetch_sched_passed') {
	...
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchALL() as $j){
			...
			echo '<tr style="cursor:pointer;  class="modal-trigger" data-toggle="modal" data-target="#passed" onclick="get_passed_details(&quot;'.$j['id'].'~!~'.$j['training_code'].'~!~'.$j['shift'].'~!~'.$j['start_date'].'~!~'.$j['end_date'].'~!~'.$j['start_time'].'~!~'.$j['end_time'].'~!~'.$j['location'].'~!~'.$j['trainer'].'~!~'.$j['slot'].'&quot;)">';
				echo '<td>'.$c.'</td>';
...
if ($method == 'fetch_prev_passed') {
	$tr_code = $_POST['tr_code'];
	$c = 0;
	$query = "SELECT *,TIME_FORMAT(start_time, '%h:%i:%s') as start_time,TIME_FORMAT(end_time, '%h:%i:%s') as end_time FROM trs_renewal_request WHERE tr_code = '$tr_code' AND exam_status = 'Passed'";
	$stmt = $conn->prepare($query);
	$stmt->execute();
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchALL() as $j){
