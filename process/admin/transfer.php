...
if ($method == 'fetch_sched_transfer_admin') {
	$start = $_POST['start'];
	$shift = $_POST['shift'];
	$c = 0;
	...
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchALL() as $j){
			...
			echo '<tr style="cursor:pointer;  class="modal-trigger" data-toggle="modal" data-target="#transfer_admin" onclick="get_transfer_admin_details(&quot;'.$j['id'].'~!~'.$j['training_code'].'~!~'.$j['shift'].'~!~'.$j['start_date'].'~!~'.$j['end_date'].'~!~'.$j['start_time'].'~!~'.$j['end_time'].'~!~'.$j['location'].'~!~'.$j['trainer'].'~!~'.$j['slot'].'&quot;)">';
				echo '<td>'.$c.'</td>';
...
if ($method == 'fetch_prev_transfer_admin') {
	$tr_code = $_POST['tr_code'];
	$c = 0;
	$query = "SELECT * FROM trs_renewal_request WHERE tr_code = '$tr_code' AND final_status = 'Transfer' AND ft_status = 0 AND exam_status = 'Failed'";
	$stmt = $conn->prepare($query);
	$stmt->execute();
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchALL() as $j){
