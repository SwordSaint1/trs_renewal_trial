...
if ($method == 'fetch_expired') {
	...
	$c = 0;
	$query = "SELECT * FROM trs_renewal_request WHERE name LIKE '$name%' AND process LIKE '$process%' AND status = 'Qualified' AND exam_status IS NULL AND expiration_on_month < '$server_date_only'";
